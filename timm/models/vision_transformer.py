...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)  #多头注意力机制计算
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity() #对注意力结果进行随机深度 Dropout 处理
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, block_layers=[], get_tokens=False, local_id = [], side_length = 7): # local_id: 数组类型的可选参数，默认为空列表。用于指定局部id
        x = self.patch_embed(x) # 转换成patch嵌入
        ...
        if local_id != []:  # 若local_id不为空，则对x进行一系列的局部区域切割和重组操作：
            if x.shape[0] != 1:
            ...
            for id in local_id:
                row_id = int(id / S)
                column_id = id % S
                # if row_id - h_S >=0 and row_id + h_S <S and column_id - h_S >=0 and column_id + h_S <S:
                    
                # else:
                row_1, row_2, column_1, column_2 = row_id - h_S, row_id + h_S + 1, column_id - h_S, column_id + h_S + 1
                if row_1 < 0:
                    row_2 = row_2 - row_1
                    row_1 = 0
                if row_2 > S:
        ...
        if get_tokens:
            ...
            for block_id, block in enumerate(self.blocks):
                x = block(x)
                if block_id in block_layers: # 如果当前block的block_id在block_layers列表中，则将经过self.norm的结果添加到x_list列表中
                    if local_id == []:
                        # print(self.norm)
            ...
            x = self.blocks(x)
        x = self.norm(x)
        if self.dist_token is None: #如果dist_token为空，则只返回x的第一个元素（预测结果）
