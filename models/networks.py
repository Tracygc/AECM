    ...
    def forward(self, x, nce_layers=[], encode_only=False):
        ...
        if len(nce_layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                try:
                    feat = layer(feat)
                    print(f"[layer {layer_id}] {layer.__class__.__name__} → {feat.shape}")
                    print(f"    当前显存: {torch.cuda.memory_allocated() / 1024 / 1024:.2f} MB")
                    print(f"    峰值显存: {torch.cuda.max_memory_allocated() / 1024 / 1024:.2f} MB")
                except Exception as e:
                    print(f"🔥 [layer {layer_id}] {layer.__class__.__name__} 报错: {e}")
                    raise e
                if layer_id in nce_layers:
                    feats.append(feat)
                if layer_id == nce_layers[-1] and encode_only:
