...
class StyleLoss(nn.Module):
    ...
    def compute_gram(self, x):
        b, ch, h, w = x.size
        f = x.view(b, ch, w * h)
        f_T = f.transpose(1, 2)   # 第2、3维度转置
        G = f.bmm(f_T) / (b * h * w * ch)      # f f_t 两个tensor矩阵乘法    (b,ch,ch)=f.bmm(f_T)shape
        return G
...
class PatchSim(nn.Module):
    ...
    def forward(self, feat, patch_ids=None):
        ...
        patch_sim = query.bmm(key) if self.use_norm else torch.tanh(query.bmm(key)/10)  # .bmm()矩阵乘法 [2, 256, 1024]用query特征图计算得到的sim map
    ...
    def select_patch(self, feat, patch_ids=None):
        ...
        feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2)  # B*N*C  合并W H
        if self.patch_nums > 0:
            if patch_ids is None:
                patch_ids = torch.randperm(feat_reshape.size(1), device=feat.device)  # 随机返回一个数组
                patch_ids = patch_ids[:int(min(self.patch_nums, patch_ids.size(0)))]  # 截取前256个值
            feat_query = feat_reshape[:, patch_ids, :]       # B*Num*C  [2, 256, 128] 被选出用来计算sim map的特征图
            feat_key = []
            ...
            if pw < W and ph < H:
                ...
                feat_key = feat.reshape(B, C, W*H)  #  B*C*N
        else:
...
class SpatialCorrelativeLoss(nn.Module):
    ...
    def cal_sim(self, f_src, f_tgt, f_other=None, layer=0, patch_ids=None):
        ...
        if self.use_conv:
            if not self.conv_init:
                self.create_conv(f_src, layer)  # create the 1*1 conv filter to select the features for a specific task
    ...
    def compare_sim(self, sim_src, sim_tgt, sim_other):
        ...
        if self.loss_mode == 'info' or sim_other is not None:  # 计算contrastive loss
            ...
            if self.loss_mode == 'l1':
                ...
                sim_pos = F.cosine_similarity(src, tgt, dim=-1)  # 余弦相似性
                # dim = -1 按行计算
    ...
    def loss(self, f_src, f_tgt, f_other=None, layer=0):
        ...
        loss = self.compare_sim(sim_src, sim_tgt, sim_other)  # measure the shape distance between the same shape and different inputs
        return loss
